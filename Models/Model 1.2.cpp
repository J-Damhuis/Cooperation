...
const int nGenerations = 2e6;					//Number of generations
...
const int nSimulations = 1;					//Number of simulations
const double threshold = 0.2;					//Threshold of responsiveness when simulation should stop
mt19937_64 rng;
...
int main()
{
	try {
		...
		for (int count = 1; count <= nSimulations; ++count) {
			...
			for (int g = 1; g <= nGenerations; ++g) {
				...
				for (int nPop = 0; nPop < nPopulations; ++nPop) {
					...
					//Calculate current fraction responsives
					double info = 0.0;
					for (int i = 0; i < n; ++i) {
						info += Populations[nPop][i].info;
					}
